...
-(void) setStyle:(FlatButtonStyle) bStyle
{
    self.currentButtonStyle = bStyle;
}

-(void) setType:(FlatButtonType) type
{
...
- (void)animateToType:(FlatButtonType)finalType {
    ...
    switch (finalType) {
        ...
        case buttonOkType:
            ...
            [self.secondSegment moveToState:doubleSegmentDownArrow animated:self.animateToStartPosition];
            firstOriginPoint.y 	+= self.bounds.size.width/6 - 5;
            secondOriginPoint.y += self.bounds.size.width/2 - 5;
